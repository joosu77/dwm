...
enum { SchemeNorm, SchemeFull, SchemeCharging, SchemeDischarging, SchemeLow, SchemeUnknown, SchemeSel };
...
copyvalidchars(char *text, char *rawtext)
{
       	...
       	while(rawtext[++i]) {
               	if ((unsigned char)rawtext[i] >= ' ' || ( rawtext[i] <= sizeof colors / sizeof colors[0] && rawtext[i] > 0 )) {
                       	text[j++] = rawtext[i];
		}
	}
	text[j] = '\0';
...
drawbar(Monitor *m)
{
	/* drw_text(drw, x, y, width, height, lpad, text, invert) */
	int x, w, tw = 0;
	...
	//if (m == selmon) { /* status is only drawn on selected monitor */
		...
		while (ctr >= 0){
			*buf = *(ts+ctr);
			if (*(ts+ctr) < 10) drw_setscheme(drw, scheme[*(ts+ctr)-1]);
			else
				drw_text(drw, m->ww - ++ctr2 * (TEXTW(buf)- lrpad), 0, TEXTW(buf) - lrpad , bh, 0, buf, 0);
			ctr--;
		}
...
updatestatus(void)
{
	...
	if (!gettextprop(root, XA_WM_NAME, rawstext, sizeof(rawstext)))
		strcpy(stext, "LMAO su statusbar ei tööta  dwm-"VERSION);
	else
